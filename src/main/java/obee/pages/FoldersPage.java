...
public class FoldersPage extends MasterPage {

	private List<ShowingCard>[] subFolderList = new List[4];
	private ArrayList<ShowingCard> usingList ;
	private User user = mongo.getUser(getUserName());
	private CardSelectionPanel usingPanel;
	private CardSelectionPanel[] subFolderPanel= new CardSelectionPanel[4];
	private Form<Object> form;
	private CheckBox[] checkBoxes = new CheckBox[4];
	private OnChangeAjaxBehavior[] cbChecked= new OnChangeAjaxBehavior[4];
	...
	private void initComponents() {
		...
		subFolderPanel[0].setTitleVisible(true);
		subFolderPanel[0].setTitle("moj tajtl");
		form.add(usingPanel);
	...
	private void initBehaviours() {
		...
		usingPanel.listChooser.addEventListener(trejder);
		for (int i = 0; i < subFolderPanel.length; i++) {
			subFolderPanel[i].listChooser.addEventListener(trejder);
		}
